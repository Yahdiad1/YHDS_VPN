...
echo -e "${GREEN}YHDS-VPN Full Installer${NC}"
...
apt install -y wget curl unzip jq git socat cron nginx openssl ca-certificates \
 ...
 figlet lolcat bc sed awk coreutils
...
# helper: create/update A record on Cloudflare
cf_create_a(){
...
# create xray config (local WS backends)
...
echo "Run 'menu.sh' or logout/login to auto-start menu."
